  ...
  def run
    ...
    File.open('/proc/stat', 'r').each_line do |line|
      ...
      if name.match(/cpu([0-9]+|)/)
        ...
        info.size.times { |i| output "#{config[:scheme]}.cpu.#{name}.#{cpu_metrics[i]}", info[i] }
      end

      output "#{config[:scheme]}.cpu.#{name}", info.last if other_metrics.include? name
    end
    if cpu_count > 0
      # writes the number of cpus, the minus 1 is because /proc/stat/
      # first line is a "cpu" which is stats for total cpus
      output "#{config[:scheme]}.cpu.cpu_count", cpu_count - 1
    end
