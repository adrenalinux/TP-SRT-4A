...
#      @warnings << "#{dev.name}: No self-tests have been logged."
    ...
    unless dev.str[0]['status'] == 'Completed without error' || dev.str[0]['status'] =~ /Self-test routine in progress/ || dev.str[0]['status'] == 'Completed'  || dev.str[0]['status'] =~ /Self test in progress(.*)/
