...
class SnfsDisk < Sensu::Plugin::Metric::CLI::Graphite
  option :host,
  ...
  def run
    ...
    filesystems.each do |fs|
      qustat_output = execute("#{config[:cvfsdir] + "/bin/qustat -f " + fs.to_s + " -m all -F all -F csv"}")
  ...
  def disk_tables(data_tables)
    @disk_tables = data_tables.select { |table| table[:table_type] == 'disk' }
  end
  ...
  def output_values(data_raw)
    ...
    disk_tables(data[:data]).each do |disk_table|
      prefix = [
        "#{config[:scheme]}",
        "snfs",
        data[:common_data][:group],
        disk_table[:table_type],
        disk_table[:table_attribute_1],
        disk_table[:table_attribute_2],
        disk_table[:table_data_type]
      ].join('.')
      disk_table[:table_data].each do |data_line|
        display_line(prefix, data[:common_data][:recorded], data_line)
      end
      output [prefix, 'last_reset', 'timestamp'].join('.'), data[:table_time_last_reset], data[:common_data][:recorded]
    end
  end
end
